...
# 加载数据
data = pd.read_csv('问题2/损失优化/关键信息提取结果（完整信息）.csv')
...
# 保存分箱边界用于后续转换
import pickle
with open('问题2/损失优化/Y染色体分箱边界.pkl', 'wb') as f:
...
resampled_df.to_csv('问题2/损失优化/SMOTE过采样数据.csv', index=False)
print('过采样数据已保存至问题2/损失优化/SMOTE过采样数据.csv')
