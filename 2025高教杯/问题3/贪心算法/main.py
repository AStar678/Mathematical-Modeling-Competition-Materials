...
# 加载数据
df = pd.read_csv('问题3/贪心算法/关键信息提取结果(PCA).csv')
...
# 修复列名问题（核心修改）
df.rename(columns={
    '18号染色体的Z值': '染色体18号Z值',  # 移除空格，统一列名
    '在参考基因组上比对的比例': '参考基因组比对比例'  # 简化名称避免潜在问题
}, inplace=True)
...
# 修正保存路径
if not os.path.exists('表格结果/问题3'):
    os.makedirs('表格结果/问题3')
results.to_csv('表格结果/问题3/模型权重.csv', index=False)
...
print(f"主成分1: {weights['主成分1']:.6f}")
print(f"主成分1_2: {weights['主成分1_2']:.6f}")
print(f"主成分1_3: {weights['主成分1_3']:.6f}")
...
# 保存汇总结果到CSV
output_csv = os.path.join('表格结果/问题3', '段划分结果汇总.csv')
