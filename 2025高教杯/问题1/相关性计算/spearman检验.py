...
# 计算指定列与 Y 染色体浓度的 spearman 相关系数和 p 值
...
result = []
for col in columns:
    corr, p_value = spearmanr(df[col], df['Y染色体浓度'])
    result.append([col, corr, p_value])

# 将结果转换为 DataFrame 并输出
result_df = pd.DataFrame(result, columns=['Column', 'Spearman_Correlation', 'P_Value'])
...
result_df.to_csv('表格结果/问题1/spearman检验结果.csv', index=False)
print(f"结果已保存到表格结果/问题1/spearman检验结果.csv")
