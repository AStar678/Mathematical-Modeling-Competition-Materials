...
def main(alpha):
    # 加载模型优化结果数据
    model_result = pd.read_csv(f'问题2/损失优化/损失优化结果/损失优化模型结果_{alpha}.csv')
    ...
    # 将结果保存为csv文件
    csv_path = f'表格结果/问题2/损失函数优化结果/损失优化模型结果_{alpha}_反归一化_含孕妇BMI.csv'
    model_result.to_csv(csv_path)

if __name__ == '__main__':
    os.makedirs('表格结果/问题2/损失函数优化结果', exist_ok=True)
    alpha = [0.1,1,10]
    for a in alpha:
        main(a)
