import pandas as pd
import numpy as np  # 新增：用于生成阈值序列
from sklearn.model_selection import train_test_split
from sklearn.ensemble import RandomForestClassifier
from sklearn.metrics import (classification_report, accuracy_score, 
                            confusion_matrix, fbeta_score)  # 新增：fbeta_score
...
accuracy_df.to_csv(f'{result_dir}/随机森林模型准确率.csv', index=False)

# 2. 保存混淆矩阵
cm = confusion_matrix(y_test, y_pred)
cm_df = pd.DataFrame(cm, index=['真实负例', '真实正例'], columns=['预测负例', '预测正例'])
cm_df.to_csv(f'{result_dir}/随机森林混淆矩阵.csv')

# 3. 保存分类报告
class_report = classification_report(y_test, y_pred, output_dict=True)
class_report_df = pd.DataFrame(class_report).transpose()
class_report_df.to_csv(f'{result_dir}/随机森林分类报告.csv')
...
summary_df.to_csv(f'{result_dir}/随机森林模型评估综合指标.csv', index=False)

# threshold_df = pd.DataFrame({
#     '最优阈值': [optimal_threshold],
#     '最优F2分数': [max_f2_score]
# })
# threshold_df.to_csv(f'{result_dir}/最优阈值与F2分数.csv', index=False)
...
plt.title('随机森林特征重要性', fontproperties=font, fontsize=16, pad=20, fontweight='bold')
...
plt.tight_layout()
plt.savefig(output_dir + '/随机森林特征重要性.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('随机森林混淆矩阵', fontproperties=font, fontsize=16, pad=20, fontweight='bold')
plt.xticks(fontproperties=font, fontsize=12)
plt.yticks(fontproperties=font, fontsize=12)

plt.tight_layout()
plt.savefig(output_dir + '/随机森林混淆矩阵.png', dpi=300, bbox_inches='tight')
plt.close()