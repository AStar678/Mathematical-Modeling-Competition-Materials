...
#from 问题3.贪心算法.随机扰动 import model_perturbed  # 用于计算差距

DATA_PATH_SMOTE = "问题2/损失优化/SMOTE过采样数据.csv"
...
def target_train(Model,perturb_std=0.01):
    # 加载数据
    ...
    for epoch in range(10):
        for x in xs_perturbed:
            ...
            loss.backward()
            optimizer.step()
        print(f"扰动 Target Epoch {epoch}, Loss: {loss.item()}")
    ...
    result.to_csv("表格结果/问题2/损失优化模型结果.csv", index=False)
    ...
    result_perturbed.to_csv("表格结果/问题2/损失优化模型扰动后结果.csv", index=False)
...
if __name__ == '__main__':
    ...
    target_train(Model,perturb_std=0.1)
